...
type MoveEventPage = Awaited<ReturnType<SuiClient['queryEvents']>>;
type MoveEvent = MoveEventPage['data'][number];

const getEventTypes = (packageId: string) => ({
...
const decodeBytes = (value: unknown): string | null => {
  if (!Array.isArray(value)) return null;
  const bytes = value.filter((item): item is number => typeof item === 'number');
  return decoder.decode(Uint8Array.from(bytes));
};

const bytesToHex = (value: unknown): string | null => {
  if (!Array.isArray(value)) return null;
  const bytes = value.filter((item): item is number => typeof item === 'number');
  return `0x${bytes.map((byte) => byte.toString(16).padStart(2, '0')).join('')}`;
};

const readFields = (object: SuiObjectResponse) => {
...
const eventPayload = (event: MoveEvent): Record<string, unknown> =>
  (event.parsedJson ?? {}) as Record<string, unknown>;

const parseBounty = (object: SuiObjectResponse, txDigest = 'direct-chain'): Bounty | null => {
  const fields = readFields(object);
  if (!fields) return null;

  return {
    id: asString((fields.id as { id?: string } | undefined)?.id),
    creator: asString(fields.creator),
    target: asString(fields.target),
    rewardAmount: asNumber(fields.reward_amount),
    status: asNumber(fields.status) as Bounty['status'],
    ...
    claimedAt:
      fields.claimed_at === null || fields.claimed_at === undefined
        ? null
...
const parseHunterBadge = (object: SuiObjectResponse, rank = 0): Hunter | null => {
  const fields = readFields(object);
  if (!fields) return null;

  return {
    address: asString(fields.hunter),
    badgeId: asString((fields.id as { id?: string } | undefined)?.id),
    ...
    rank,
    lastKillAt:
      fields.last_kill_at === null || fields.last_kill_at === undefined
        ? null
        : asNumber(fields.last_kill_at),
  };
};

const sortBounties = (items: Bounty[], filter: BountyFilter): Bounty[] => {
...
const recentEventsForType = async (type: string, limit: number): Promise<MoveEvent[]> => {
...
const recentEventsUntilMatch = async (
  type: string,
  predicate: (event: MoveEvent) => boolean,
  pageSize = 40,
  pageBudget = 4,
...
const loadCreationTxDigest = async (
  bountyId: string,
  bountyCreatedType: string,
): Promise<string> => {
...
const fetchBoardFields = async (boardId: string): Promise<Record<string, unknown>> => {
...
const fetchClaimRegistryFields = async (
  claimRegistryId: string,
...
const fetchBountyObjects = async (ids: string[], bountyCreatedType: string): Promise<Bounty[]> => {
  ...
  const createEventMap = new Map<string, string>();
  const createdEvents = await recentEventsForType(
    bountyCreatedType,
    Math.min(EVENT_LIMIT, Math.max(ids.length * 3, 12)),
  );
...
const filterBounties = (items: Bounty[], filter: BountyFilter): Bounty[] => {
  return items.filter((item) => {
    if (filter.status !== undefined && item.status !== filter.status) return false;
    if (filter.target && normalizeAddress(item.target) !== normalizeAddress(filter.target)) return false;
    if (filter.creator && normalizeAddress(item.creator) !== normalizeAddress(filter.creator)) return false;
    return true;
  });
};

const mapClaimEvent = (event: MoveEvent): ClaimRecord => {
  const payload = eventPayload(event);
  return {
...
class ChainDataService {
  async getBounties(filter: BountyFilter = {}): Promise<Bounty[]> {
    ...
    const createdEvents = await recentEventsForType(
      eventTypes.bountyCreated,
      Math.max(EVENT_LIMIT, pageSize * 4),
    );
    ...
    const bounties = filterBounties(
      await fetchBountyObjects(uniqueIds, eventTypes.bountyCreated),
      filter,
    );
  ...
  async getHunter(address: string): Promise<Hunter | null> {
    ...
    const badges = objects.data
      ...
      .filter((item): item is Hunter => Boolean(item))
      .sort((left, right) => {
  ...
  async getLeaderboard(limit = 50): Promise<Hunter[]> {
    const config = requireFrontendConfig();
    const eventTypes = getEventTypes(config.packageId);
    ...
    return [...hunterMap.values()]
      .sort((left, right) => {
        ...
        if (right.totalEarnings !== left.totalEarnings) return right.totalEarnings - left.totalEarnings;
        return (right.lastKillAt ?? 0) - (left.lastKillAt ?? 0);
      })
      .slice(0, limit)
  ...
  async getStats(): Promise<StatsSnapshot> {
    ...
    const usedProofs = claimRegistryFields.used_proofs as { fields?: { size?: string | number } } | undefined;
    const targetRewardTable = boardFields.target_total_active_reward as {
  ...
  async getRecentEvents(limit = 20): Promise<IndexedEvent[]> {
    const config = requireFrontendConfig();
    const eventTypes = getEventTypes(config.packageId);
