...
export type FrontendNetworkName = 'testnet' | 'devnet' | 'mainnet' | 'localnet';
export type FrontendDataMode = 'auto' | 'indexer' | 'chain-direct';
...
export interface FrontendAppConfig {
  ...
  killProofIssuerCapId: string;
  clockId: string;
  indexerApiUrl: string;
  indexerWsUrl: string;
  indexerHealthTimeoutMs: number;
  localnetUrl: string;
}

export interface FrontendConfigDiagnostics {
  isValid: boolean;
  errors: string[];
...
const allowedNetworks = new Set<FrontendNetworkName>(['testnet', 'devnet', 'mainnet', 'localnet']);
const allowedModes = new Set<FrontendDataMode>(['auto', 'indexer', 'chain-direct']);

const clean = (value: string | undefined): string => value?.trim() ?? '';

const parseId = (name: string, value: string, errors: string[]): string => {
  if (!value) {
...
export function validateFrontendEnvRecord(
  env: Record<string, string | undefined>,
): FrontendConfigDiagnostics {
  const errors: string[] = [];
  const raw = {
  ...
  const network = allowedNetworks.has(raw.VITE_SUI_NETWORK as FrontendNetworkName)
    ? (raw.VITE_SUI_NETWORK as FrontendNetworkName)
    : 'testnet';
  if (!allowedNetworks.has(raw.VITE_SUI_NETWORK as FrontendNetworkName)) {
    errors.push(`VITE_SUI_NETWORK must be one of testnet, devnet, mainnet, localnet. Received: ${raw.VITE_SUI_NETWORK}`);
  }

  const dataMode = allowedModes.has(raw.VITE_DATA_MODE as FrontendDataMode)
    ? (raw.VITE_DATA_MODE as FrontendDataMode)
    : 'auto';
  if (!allowedModes.has(raw.VITE_DATA_MODE as FrontendDataMode)) {
  ...
  const killProofIssuerCapId = parseId(
    'VITE_KILL_PROOF_ISSUER_CAP_ID',
    raw.VITE_KILL_PROOF_ISSUER_CAP_ID,
    errors,
  );
  ...
  const config: FrontendAppConfig | null = errors.length
    ? null
    : {
        network,
        dataMode,
...
export function formatFrontendConfigDiagnostics(
